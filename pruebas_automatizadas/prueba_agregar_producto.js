...
(async () => {
  ...
  try{
    ...
    await page.waitForSelector('#Agregar_producto',{ visible: true, timeout: 5000 }); 
    await page.click('#Agregar_producto');
    
    await page.waitForSelector('#Formulario_agregar',{ visible: true, timeout: 5000 });
  
    await page.waitForSelector('#nombre_producto', { visible: true, timeout: 5000 }); 
    await page.type('#nombre_producto', 'producto de prueba');
    
    await page.waitForSelector('#descripcion_producto', { visible: true, timeout: 5000 }); 
    ...
    await page.waitForSelector('#precioInicial', { visible: true, timeout: 5000 }); 
    await page.type('#precioInicial', '1000');
  
    await page.waitForSelector('#duracion_remate', { visible: true, timeout: 5000 }); 
    await page.type('#duracion_remate', '2');
  
    await page.waitForSelector('#categoria_producto', { visible: true, timeout: 5000 }); 
    await page.select('#categoria_producto', 'tecnologia');
  
    await page.waitForSelector('#submitAgregarProducto', { visible: true, timeout: 5000 }); 
